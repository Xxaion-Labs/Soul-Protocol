...
REQUIRED = {
    ...
    'README.md': ['⧉', 'a tesseract', 'The current public compatibility surface is `.doctrine`.'],
    'SPEC.md': ['⧉', 'a tesseract', 'Semantic Face', 'Machine Face', 'Mount Face', 'Proof Face'],
    'COMPATIBILITY.md': ['⧉ compatible', '.doctrine compatible'],
}
...
def main() -> int:
    ...
    if failures:
        print('Tesseract standard check failed:')
        for failure in failures:
            print(f'- {failure}')
        return 1
    print('Tesseract standard check passed.')
    return 0
